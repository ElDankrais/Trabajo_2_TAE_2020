...
```{r, warning=FALSE, message=FALSE, fig.height=8, fig.width=8.5}
library(ISLR)
data(Weekly)
attach(Weekly)
rbind(head(Weekly,10))
summary(Weekly)
ggpairs(Weekly, aes(colour = Direction, alpha = 0.5),
        upper = list(continuous = wrap('cor', size = 2.4)),
title = "Matriz de dispersión") +
theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```

Los valores en rojo corresponden al nivel *Down* de la variable categórica **Direction** y los valores azules al nivel *Up*. Esto es útil cuando se quiere observar su ambas poblaciones tienen el mismo comportamiento estando juntas y estando separadas.

Podemos comprobar que la única gran correlación se da entre las variables año y volumen. Además, su relación no es lineal ya que se aprecia una curva. También se observa que ninguna combinación de las demás variables logra tener una correlación mayor o igual al 10% combinando los datos de la población *Down* y la población *Up*.

Se observa que todos los rezagos se concentran en el valor de cero gracias a la densidad que nos enseña el gráfico. Además, la variable **today** separa muy bien las poblaciones *Down* y *Up*. También se nota que no hay muchas diferencias entre ambas poblaciones respecto a variables diferentes a **today**.
...
mosaic(matriz.confusion, shade = T, colorize = T, 
       gp = gpar(fill = matrix(c("2", "4", "4", "2"), 2, 2)))
```
...
<span class="negrilla"> * LDA - Lag1 & Lag2: </span>
...
<span class="negrilla"> * LDA Interacción - Lag1 & Lag2: </span>
...
<span class="negrilla"> * QDA - Lag2 & sqrt(Lag2): </span>
...
<span class="negrilla"> * QDA - [Lag2 & sqrt(Lag2)] & [Lag1 & sqrt(Lag1)]: </span>
...
<span class="negrilla"> * Regresión logística - Lag1 & Lag2: </span>
...
<span class="negrilla subrayar">Using the Boston data set, fit classification models in order to predict whether a given suburb has a crime rate above or below the median. Explore logistic regression, LDA, and KNN models using various subsets of the predictors. Describe your findings.</span>
...
pairs(Boston, lower.panel = myPanel.cor, upper.panel = panel.smooth, diag.panel = myPanel.box, labels = names(Boston))

corrplot.mixed(cor(Boston), order="hclust", tl.col="black")
```

```{r, warning=FALSE}
set.seed(23)
train <- createDataPartition(y = Boston$crimen.class, p = 0.7, list = FALSE, times = 1)
Boston.train = Boston[train,]
Boston.test = Boston[-train,]
crimen.class.test = crimen.class[-train]
```

<span class="negrilla"> * Regresión Logística: </span>
...
<span class="negrilla"> * Regresión Logística (sin ptratio, chas, black, rm, medv y zn): </span>

```{r, warning=FALSE}
#REGRESION LOGISTICA 2
logist.fit = glm(crimen.class ~ . -crimen.class -crim -ptratio -chas -black -rm -medv -zn ,
...
Para ambos casos de regresión logística la varaible dependiente utilizada fue la de crimen, pero, mientras que en la primera se utilizaron todas las demás, durante el segundo modelo se quitaron ptratio, chas, black, rm, medv y zn debido a que estas presentan muy poca correlación con la varaible crimen. La tasa del error para le primer modelo fue de 8.666667%, mientras que para el segundo fue de 10.66667%. En consecuencia, se llegó a que el primer modelo fue el mejor predictor.

<span class="negrilla"> * LDA (todos): </span>
...
<span class="negrilla"> * LDA (sin ptratio, chas, black, rm, medv y zn): </span>

```{r, warning=FALSE}
lda.fit = lda(crimen.class ~ . -crimen.class -crim -ptratio -chas -black -rm -medv -zn,
                 data = Boston, subset = train)
lda.pred = predict(lda.fit, Boston.test)
mean(lda.pred$class != crimen.class.test)*100
```

<span class="negrilla"> * LDA (sin zn, chas, tax y nox): </span>

```{r, warning=FALSE}
lda.fit = lda(crimen.class ~ . -crimen.class -crim -zn -tax -nox,
                 data = Boston, subset = train)
lda.pred = predict(lda.fit, Boston.test)
mean(lda.pred$class != crimen.class.test)*100
```

<span class="negrilla"> * LDA (sin zn, crim, chas y rm): </span>

```{r, warning=FALSE}
lda.fit = lda(crimen.class ~ . -crimen.class -crim -chas -rm ,
                 data = Boston, subset = train)
lda.pred = predict(lda.fit, Boston.test)
mean(lda.pred$class != crimen.class.test)*100
```

Se crearon varios modelos para intentar reducir el error utilizando LDA, pero ninguno de los 3 intentos distintos dieron un resultado satisfactorio ya que el primero resultó ser el mejor de todos con un error del 12%.
...
<span class="negrilla"> * KNN (K = 1): </span>
...
<span class="negrilla"> * KNN (K = 5): </span>

```{r, warning=FALSE}
knn.pred = knn(train.X, test.X, train.crimen.class, k = 5)
mean(knn.pred != crimen.class.test)*100
```

<span class="negrilla"> * KNN (K = 10): </span>
...
<span class="negrilla"> * KNN (K = 50): </span>

```{r, warning=FALSE}
knn.pred = knn(train.X, test.X, train.crimen.class, k = 50)
mean(knn.pred != crimen.class.test)*100
```

<span class="negrilla"> * KNN (K = 100): </span>
...
Se tomaron modelos para diferentes valores de K, contando con todas las variables presentes (K=1, K=5, K=10, K=50 Y K=100). Luego de llegar a todos los resultados, se observó que k=5 mostraba una mejor precisión, con un porcentaje de error de tan solo 6.666667%.
...
Luego de hacer las predicciones para un subconjunto de las varaibles, se obtuvo que para  K= 1 el error era de 13.86% , mientras que para K = 10 era de 14.85%.
...
<span class="negrilla">GRÁFICAS CON EL MEJOR MODELO SVM POLINOMIAL</span>
...
<span class="negrilla">GRÁFICAS CON EL MEJOR MODELO SVM RADIAL</span>
